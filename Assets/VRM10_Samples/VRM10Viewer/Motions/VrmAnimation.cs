...
namespace UniVRM10.VRM10Viewer
{
    public class VrmAnimation : IMotion
    {
        ...
        static (Vector3, Dictionary<HumanBodyBones, Quaternion>) GetPose(JsonNode frame)
        {
            ...
            foreach (var kv in frame.ObjectItems())
            {
                switch (kv.Key.GetString())
                {
                    case "translation": root = ToVec3(kv.Value); break;
        ...
        public static async Task<VrmAnimation> LoadVrmAnimationPose(string text)
        {
            ...
            if (data.GLTF.extensions is UniGLTF.glTFExtensionImport extensions)
            {
                foreach (var kv in extensions.ObjectItems())
                {
                    ...
                    {
                        if (kv.Value.TryGet("humanoid", out var humanoid))
                        {
                            if (humanoid.TryGet("frame", out var frame))
                            {
                                var (root, map) = GetPose(frame);
