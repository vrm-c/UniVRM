...
namespace VRM.SpringBone
{
    ...
    readonly struct SpringBoneJointInit
    {
        ...
        public Vector3 VerletIntegration(float deltaTime, Transform center, Quaternion parentRotation,
            SpringBoneSettings settings, SpringBoneJointState _state, float scalingFactor, Vector3 external)
        {
            ...
            var nextTail = state.CurrentTail
                           ...
                           + (settings.GravityDir * settings.GravityPower + external) * deltaTime * scalingFactor; // 外力による移動量
            return nextTail;
        }
